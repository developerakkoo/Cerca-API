  ...
  new Worker(
    ...
    async job => {
      try {
        ...
        console.log(`🔍 Processing ride ${ride._id} | status: ${ride.status}`)

        // Only process requested rides
        if (ride.status !== 'requested') {
          console.log(`⚠️ Ride ${ride._id} skipped (status: ${ride.status})`)
          return
        }
        ...
        for (const driver of drivers) {
          if (!driver.socketId) {
            console.log(`⚠️ Driver ${driver._id} skipped (no socketId)`)
            continue
          }

          console.log(`📡 Sending ride ${ride._id} to driver ${driver._id}`)
