...
const assignDriverToRide = async (rideId, driverId, driverSocketId) => {
  try {
    ...
    //   logger.info('🔓 Driver lock released', { lockKey })

    // 🔓 STEP 5: Release driver lock after successful assignment
await redis.del(lockKey)

logger.info('🔓 Driver lock released after assignment', {
  lockKey,
  driverId,
  rideId
})
    ...
    if (ride.bookingType && ride.bookingType !== 'INSTANT') {
      ...
      if (endTime) {
        ...
        logger.info('⏱️ Driver lock TTL extended', {
          lockKey,
          ttl
        })
      }
